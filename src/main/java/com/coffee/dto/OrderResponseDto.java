...
public class OrderResponseDto {
    private Long orderId; // 송장 번호(주문의 고유번호_PK 기반)
    private LocalDate orderDate; //  주문 날짜
    private String status; // 주문의 상태

    // 주문에 속해있는 상품 목록을 의미, 하단의 OrderItem에 대한 컬렉션
    private List<OrderItem> orderItems;

    // OrderProduct 엔터티와 이름이 동일하여 OrderItem으로 명명
    // OrderProduct 클래스는 '주문 상품' 1개를 의미
    @Data
    @AllArgsConstructor // 만약 상품에 대한 추가 정보가 더 필요하면 하단에 변수 추가하기
