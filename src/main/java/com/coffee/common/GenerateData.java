...
// 테스트 용도로 '회원', '상품' 등의 임시 데이터를 생성하기 위한 자바 클래스
public class GenerateData {
    //주의) SpringBoot가 아닌 일반 Test 모드에서는 @Value를 사용할 수 없음
    private final String imageFolder = "c:\\shop\\images";

    //특정 폴더 내에 들어있는 모든 이미지의 파일 이름을 List 컬렉션으로 반환.
    public List<String> getImageFileNames() {
        // File : 파일이나 폴더를 객체 형태로 다루고자 할 때 사용하는 클래스
        File folder = new File(imageFolder);
        List<String> imageFiles = new ArrayList<>(); // 이미지 이름들을 저장할 커

        // exits()는 해당 객체가 실제로 존재하면 true
        // isDirectory()는 해당 객체가 폴더면 true
        ...
        // 모든 파일의 이름을 소문자로 변경해서 확장자와 비교 후 조건에 부합하면 컬렉션에 추가.
        if (fileList != null) {
